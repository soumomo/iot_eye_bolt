...
const ColorWheelSelector: React.FC<ColorWheelSelectorProps> = ({
  ...
  const getColorHex = (tailwindColor: string) => {
    return tailwindToHex[tailwindColor] || '#ffffff';
  };
  ...
  const getGlow = (index: number) => {
    if (hoveredGroup === index || currentColorGroup === index) {
      const colorHex = getColorHex(colorGroups[index].color);
      return `0 0 20px ${colorHex}80, 0 0 40px ${colorHex}40`;
    }
    return 'none';
  };

  const selectedColorHex = getColorHex(colorGroups[currentColorGroup].color);
  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 pointer-events-none">
        <div 
          className="w-8 h-8 rounded-full border-2 border-white/50 shadow-lg transition-all duration-300"
          style={{ 
